...
# --- Patch: allow ticker-based workflows ---
def _resolve_tickers(self, parsed_strategy, config):
    """Return tickers list from strategy or sector."""
    if parsed_strategy.get("sector"):
        return self._get_tickers_for_sector(parsed_strategy["sector"])
    elif parsed_strategy.get("symbols"):
        return parsed_strategy["symbols"]
    else:
        return [config["backtest"]["default_symbol"]]
